...
function responseWith(outputText: string, ok = true, status = 200): Response {
...
describe("OpenAiResponsesGateway", () => {
  ...
  it("rejects a tool plan that asks for a tool the product did not authorize", async () => {
    const gateway = new OpenAiResponsesGateway({
      apiKey: "secret",
      model: "model",
      fetchImpl: (async () => responseWith('[{"tool":"unregistered_tool","args":{}}]')) as typeof fetch,
    });
  ...
  it("parses missing information without inventing a source reference", async () => {
    const gateway = new OpenAiResponsesGateway({
      apiKey: "secret",
      model: "model",
      fetchImpl: (async () => responseWith(JSON.stringify({
        intent: "utility_gap",
        sections: [{
  ...
  it("returns a concise provider error without echoing response bodies", async () => {
    const gateway = new OpenAiResponsesGateway({
      apiKey: "secret",
      model: "model",
      fetchImpl: (async () => responseWith("sensitive provider body", false, 503)) as typeof fetch,
    });
